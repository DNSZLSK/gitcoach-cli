...
describe('CopilotService', () => {
  ...
  describe('isAvailable', () => {
    it('should return true when copilot CLI is installed', async () => {
      mockExecuteFile.mockResolvedValue({ stdout: '0.0.393', stderr: '' });
      const result = await copilotService.isAvailable();
      expect(result).toBe(true);
    });

    it('should return false when copilot CLI is not found', async () => {
      mockExecuteFile.mockRejectedValue(new Error('command not found'));
      const result = await copilotService.isAvailable();
      expect(result).toBe(false);
    });

    it('should return false when stderr contains not found', async () => {
      mockExecuteFile.mockResolvedValue({ stdout: '', stderr: 'copilot not found' });
      const result = await copilotService.isAvailable();
      expect(result).toBe(false);
    });

    it('should cache the availability result', async () => {
      mockExecuteFile.mockResolvedValue({ stdout: '1.0.0', stderr: '' });
      await copilotService.isAvailable();
      await copilotService.isAvailable();
      expect(mockExecuteFile).toHaveBeenCalledTimes(1);
    });
  ...
  describe('generateCommitMessage', () => {
    ...
    it('should parse conventional commit from output', async () => {
      ...
      mockExecuteFile.mockResolvedValueOnce({
    ...
    it('should handle failed generation', async () => {
      mockExecuteFile.mockResolvedValueOnce({ stdout: '1.0.0', stderr: '' });
      mockExecuteFile.mockRejectedValueOnce(new Error('timeout'));
      // Retry also fails
      mockExecuteFile.mockRejectedValueOnce(new Error('timeout'));
  ...
  describe('installCopilotCli', () => {
    it('should return success when install succeeds', async () => {
      mockExecuteFile.mockResolvedValueOnce({ stdout: 'installed', stderr: '' });
    ...
    it('should return failure when install fails', async () => {
      mockExecuteFile.mockRejectedValueOnce(new Error('npm install failed'));
  ...
  describe('resetAvailability', () => {
    it('should clear cached availability', async () => {
      ...
      mockExecuteFile.mockResolvedValue({ stdout: '', stderr: 'not found' });
      const result = await copilotService.isAvailable();
      expect(result).toBe(false);
    });
  });
